...
def run_tests(base_path, inter_path):
    ...
    for _dir in dirs:
        ...
        if (
            ...
            print(folder)
            os.chdir(folder)
            # runs the tests by going to that directory... you may have to change python3 to python, depending on your system
    ...
    for _file in files:
        if _file.startswith("test_"):  # to handle the case of file .pytest_cache
            try:
                ...
                remarks = ""
                total_tests = int(
                    re.findall(r"collected\s(\d+)\sitems", "".join(content))[0]
                )
                passed_tests = 0
                ...
                remarks += f"Out of {total_tests} tests, your code passed {passed_tests} and failed {total_tests - passed_tests}."
                ...
                if " short test summary info " in "".join(content):
                    ...
                    failed_messages_list = (
                        "".join(content)
                        .split(" short test summary info ")[1]
                        .split("\n")[1:-2]
                    )
                    for message in failed_messages_list:
                        remarks += f"{message.split('Tests::')[1].split(' - ')[0]}, "
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Grading Python assignments based on pytest unit tests"
    )
    parser.add_argument(
        "-b",
        "--base_path",
